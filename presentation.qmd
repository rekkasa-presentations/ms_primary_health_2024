...
$$
\begin{align}
log(\frac{p_{treatment}}{1-p_{treatment}}) = &-0.34 - 0.92\times sex\_male\\
...
rct_data <- readr::read_csv("data/gusto_with_multiple_confounders.csv") |>
  dplyr::mutate(
    sex = ifelse(sex_male == 1, "male", "female"),
    treatment = ifelse(treatment == 1, "tPA", "Streptokinase")
  ) |>
...
> χρησιμοποιηθεί για να εξεταστεί η παράλειψη συγχυτικών παραγόντων από την
